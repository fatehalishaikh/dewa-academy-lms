...
export function StudentAiTutor() {
  ...
  function sendMessage() {
    ...
    setTimeout(() => {
      ...
      setMessages(prev => [...prev, {
        role: 'assistant',
        content: `That's a great question about "${input.slice(0, 40)}..."! \n\nBased on the **${selectedTopic}** topic, let me break this down for you:\n\n• The key concept here relates to the fundamentals we discussed\n• Think about how the formula/rule applies step by step\n• Try working through it methodically\n\nDo you want me to show you a worked example, or would you like to try a practice problem?`,
        timestamp: new Date(),
      }])
    }, 1200)
  }
  ...
  function submitAnswer() {
    ...
    setTimeout(() => {
      ...
      const correct = input.toLowerCase().includes(q?.answer?.split(' ').slice(-1)[0]?.toLowerCase() ?? '')
      if (selectedTopic && correct && !masteredTopics.includes(selectedTopic)) {
        setMasteredTopics(prev => [...prev, selectedTopic])
      }
      setMessages(prev => [...prev, {
        role: 'assistant',
        content: correct
          ? `✅ **Correct! Well done!**\n\nThe answer is: **${q?.answer}**\n\nExcellent work. You've demonstrated understanding of this concept. Your mastery of **${selectedTopic}** has been recorded! 🌟`
          : `Not quite, but good effort! 💪\n\nThe correct answer is: **${q?.answer}**\n\nLet me explain why:\n${q?.hint}\n\nTry the next step: apply the concept again with a fresh approach. Would you like another practice question?`,
        timestamp: new Date(),
      }])
      setMode('chat')
    }, 1500)
                ...
                <div className="p-4 space-y-3">
                  {messages.map((msg, i) => (
                    <div key={i} className={`flex gap-2.5 ${msg.role === 'user' ? 'flex-row-reverse' : ''}`}>
                      ...
                      <div className={`max-w-[85%] rounded-2xl px-3.5 py-2.5 text-xs leading-relaxed whitespace-pre-wrap ${msg.role === 'assistant' ? 'bg-card border border-border text-foreground' : 'bg-primary/10 border border-primary/20 text-foreground'}`}>
                        {msg.content}
                      </div>
                    </div>
                  ))}
