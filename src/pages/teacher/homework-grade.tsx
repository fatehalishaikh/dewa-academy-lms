...
export function HomeworkGrade() {
  const { id: homeworkId, submissionId } = useParams<{ id: string; submissionId: string }>()
  ...
  function handleSave() {
    ...
    setTimeout(() => {
      ...
      if (nextPending) {
        navigate(`/teacher/homework/${homeworkId}/grade/${nextPending.id}`)
      } else {
  ...
  function goToSubmission(dir: 'prev' | 'next') {
    ...
    if (target) navigate(`/teacher/homework/${homeworkId}/grade/${target.id}`)
  }
  ...
  return (
    <div className="h-screen flex flex-col p-6 gap-4 max-w-6xl mx-auto">
      ...
      <div className="flex items-center gap-3 shrink-0">
        <Button variant="ghost" size="sm" onClick={() => navigate(`/teacher/homework/${homeworkId}`)} className="gap-1.5 -ml-2">
