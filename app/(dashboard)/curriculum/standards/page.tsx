    ...
    <div className="space-y-4">
              ...
              return (
                ...
                >
                  ...
                  <div className="min-w-0">
                    <span className="font-mono text-[11px] text-muted-foreground block">{std.code}</span>
      ...
      {selectedNodeId && selectedStdId && (
        <div className="flex items-center gap-3 p-3 bg-muted/10 border border-border rounded-xl">
          ...
          {linkFeedback && <span className="text-[11px] text-emerald-400">Linked!</span>}
                  ...
                  .map(m => {
                    ...
                    return (
                      <div key={m.standardId} className="flex items-center gap-1 bg-muted/20 border border-border rounded-lg px-2 py-1">
                        ...
                        <span className="text-[11px] font-mono text-foreground">{std?.code ?? m.standardId}</span>
                        <button
      ...
      <Card className="rounded-2xl border-border">
        ...
        <CardContent className="p-4">
          <div className="space-y-2">
            <div className="grid grid-cols-[160px_repeat(4,1fr)] gap-2 text-[11px] font-semibold text-muted-foreground uppercase tracking-wider px-2">
              <span>Subject</span>
            ...
            {coverageData.map(row => {
              ...
              return (
                <div key={row.subject} className="grid grid-cols-[160px_repeat(4,1fr)] gap-2 items-center">
                  ...
                  <div className={`text-center text-[11px] font-semibold py-1 rounded-lg border ${coverageCell(row.covered, row.partial, 0)}`}>
                    {row.covered}
                  </div>
                  <div className="text-center text-[11px] font-semibold py-1 rounded-lg border border-amber-500/20 bg-amber-500/10 text-amber-400">
                    {row.partial}
                  </div>
                  <div className="text-center text-[11px] font-semibold py-1 rounded-lg border border-red-500/20 bg-red-500/10 text-red-400">
                    {row.gap}
                  </div>
                  <div className={`text-center text-[11px] font-semibold py-1 rounded-lg border ${assessedCell}`}>
                    {assessedLabel}
                  </div>
                </div>
              )
          ...
          <div className="flex gap-4 mt-4 text-[11px] text-muted-foreground flex-wrap">
      ...
      <Card className="rounded-2xl border-border">
        ...
        <CardContent className="p-4">
          <p className="text-[11px] text-muted-foreground mb-3">Standards are mapped across all curriculum levels — from courses down to individual activities within lessons.</p>
            ...
            {[
              ...
              <div key={item.level} className="flex items-center gap-2">
                <div className={`px-3 py-2 rounded-xl border text-center min-w-[80px] ${item.color} bg-muted/5`}>
                  ...
                  <p className="text-[11px] text-muted-foreground uppercase tracking-wide">{item.level}</p>
                </div>
