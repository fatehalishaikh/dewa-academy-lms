        ...
        <TabsContent value="overview" className="mt-6">
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
            <div className="lg:col-span-2 space-y-6">
              ...
              <Card className="rounded-[10px] border-border bg-card">
                ...
                <CardContent className="px-5 pb-5 space-y-4">
                  <div className="grid grid-cols-3 gap-3">
                    <div className="p-3 rounded-[10px] bg-muted/20 border border-border text-center">
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                    {sortedSubjects.map(cls => {
                      ...
                      return (
                        <div key={cls.classId} className={`p-3 rounded-[10px] bg-muted/20 border ${gradeBorderColor(cls.average)} ${isStruggling ? 'border-2' : ''}`}>
                          ...
                          <div className="flex items-center justify-between">
                            ...
                            <Badge variant="outline" className="text-[9px] h-4 border-border text-muted-foreground">{letterGrade(cls.average)}</Badge>
                          </div>
                        </div>
                      )
                    })}
              ...
              {teacherPublished && (
                <Card className="rounded-[10px] border-primary/30 bg-primary/5">
                  <CardHeader className="pb-3 pt-5 px-5">
                    <div className="flex items-center justify-between">
                      <CardTitle className="text-sm font-semibold flex items-center gap-2">
                  ...
                  <CardContent className="px-5 pb-5 space-y-4">
                    ...
                    <div className="space-y-2">
                      ...
                      {teacherPublished.path.focusAreas.map((fa, i) => (
                        <div key={i} className="p-3 rounded-[10px] bg-card border border-border space-y-1">
                          <div className="flex items-center justify-between">
                    ...
                    <div className="space-y-2">
                      ...
                      {teacherPublished.path.weeklyPlan.map(week => (
                        <div key={week.week} className="flex gap-3 p-3 rounded-[10px] bg-card border border-border">
                          ...
                          <div className="flex-1 min-w-0">
                            <div className="flex items-center justify-between mb-1">
                              ...
                              <span className="text-[10px] text-muted-foreground flex items-center gap-1"><Clock className="w-3 h-3" />{week.hoursRequired}h</span>
                            </div>
                            <ul className="space-y-0.5">
                              {week.activities.map((act, j) => (
                                <li key={j} className="text-[10px] text-muted-foreground flex gap-1.5"><span className="text-primary shrink-0">·</span>{act}</li>
                              ))}
                            </ul>
                          </div>
                        </div>
                    ...
                    <div className="space-y-2">
                      ...
                      {teacherPublished.path.milestones.map((m, i) => (
                        <div key={i} className="flex items-center justify-between p-2.5 rounded-[10px] bg-card border border-border">
              ...
              <Card className="rounded-[10px] border-border bg-card">
                ...
                <CardContent className="px-5 pb-5 space-y-3">
                  ...
                  {showAiSuggestions && (
                    <div className="rounded-[10px] border border-primary/20 bg-primary/5 overflow-hidden">
                      <div className="flex items-center gap-2 px-3 py-2.5 border-b border-primary/10">
                        ...
                        <p className="text-[10px] text-muted-foreground ml-auto">Based on your grades &amp; profile</p>
                      </div>
                      {loadingAiSuggestions ? (
                        ...
                        <div className="p-3 space-y-2">
                          {aiSuggestions.map((s, i) => {
                            ...
                            return (
                              <div key={i} className="flex items-start gap-2.5 p-2.5 rounded-[10px] bg-card border border-border">
                                ...
                                <div className="flex-1 min-w-0 space-y-1.5">
                                  ...
                                  <div className="flex items-center justify-between gap-2">
                                    <Badge
                                      variant="outline"
                                      className="text-[9px] h-4 border-transparent shrink-0"
                                    ...
                                    <Button
                                      ...
                                      disabled={added}
                                      className={`h-6 text-[10px] shrink-0 ${added ? 'text-emerald-400 border-emerald-500/30' : 'text-primary hover:bg-primary/10'}`}
                  ...
                  {showAddGoal && (
                    <div className="p-3 rounded-[10px] bg-muted/20 border border-primary/20 space-y-2">
                      <Input
                        value={newGoalText}
                  ...
                  {allGoals.map((g, i) => {
                    ...
                    return (
                      <div key={i} className="p-3 rounded-[10px] bg-muted/20 border border-border space-y-2">
            ...
            <div className="space-y-4">
              ...
              <Card className={`rounded-[10px] ${riskCfg.border} border ${riskCfg.bg}`}>
                <CardHeader className="pb-3 pt-5 px-5">
                  <CardTitle className="text-sm font-semibold flex items-center gap-2">
                ...
                <CardContent className="px-5 pb-5 space-y-3">
                  <div className="flex items-center justify-between">
                    ...
                    <div className="text-right space-y-1">
                      <Badge variant="outline" className={`text-[10px] h-5 ${riskCfg.color} ${riskCfg.border}`}>{riskCfg.label}</Badge>
                      {riskData && (
            ...
            <div className="lg:col-span-2 space-y-4">
              ...
              {WEEKS.map(week => {
                ...
                return (
                  <Card
                    key={week.week}
                    className={`rounded-[10px] border transition-colors cursor-pointer ${isSelected ? 'border-primary/40 bg-primary/5' : 'border-border bg-card hover:border-primary/20'}`}
                    onClick={() => setSelectedWeek(isSelected ? null : week.week)}
                  >
                    <CardContent className="p-4 space-y-3">
                      <div className="flex items-start justify-between gap-3">
                        <div className="flex items-center gap-3">
                          <div className={`w-10 h-10 rounded-[10px] flex items-center justify-center shrink-0 font-bold text-sm ${week.status === 'active' ? 'bg-primary/20 text-primary' : week.status === 'completed' ? 'bg-emerald-500/20 text-emerald-400' : 'bg-muted/40 text-muted-foreground'}`}>
                            W{week.week}
                          </div>
                          <div>
                        ...
                        <Badge variant="outline" className={`text-[10px] h-5 shrink-0 ${wsCfg.color} ${wsCfg.border} ${wsCfg.bg}`}>
                          {wsCfg.label}
                        </Badge>
                      </div>
                      ...
                      {week.status !== 'upcoming' && (
                        <div className="space-y-1">
                          <div className="flex items-center justify-between">
                            ...
                            <span className="text-[10px] font-semibold text-foreground">{week.progress}%</span>
                          </div>
                      ...
                      {isSelected && (
                        <div className="space-y-4 pt-2 border-t border-border">
                          {/* Topics */}
                          <div>
                            <p className="text-[10px] font-semibold text-muted-foreground uppercase tracking-wider mb-2">Topics This Week</p>
                            <div className="space-y-1.5">
                              {week.topics.map((t, i) => (
                                <div key={i} className="flex items-center gap-2.5 p-2 rounded-[10px] bg-muted/20 border border-border">
                          ...
                          {/* Assignments */}
                          <div>
                            <p className="text-[10px] font-semibold text-muted-foreground uppercase tracking-wider mb-2">Assignments</p>
                              ...
                              <div className="space-y-1.5">
                                {week.assignments.map((a, i) => (
                                  <div key={i} className="flex items-center gap-3 p-2.5 rounded-[10px] bg-muted/20 border border-border">
                                    ...
                                    <Badge variant="outline" className="text-[9px] h-4 border-amber-500/30 text-amber-400 shrink-0">
                                      Due {a.dueDay}
                                    </Badge>
                                  </div>
                                ))}
                  ...
                  {WEEKS.map(w => (
                    <div key={w.week} className="space-y-1">
                      <div className="flex items-center justify-between">
                        ...
                        <span className={`text-[10px] font-semibold ${weekStatusConfig[w.status].color}`}>
                ...
                <CardContent className="px-5 py-4 space-y-3">
                  ...
                  {WEEKS.flatMap(w => w.assignments.map(a => ({ ...a, week: w.week }))).map((a, i) => (
                    <div key={i} className="flex items-center justify-between gap-2">
                      ...
                      <Badge variant="outline" className="text-[9px] h-4 shrink-0 border-border text-muted-foreground">W{a.week}</Badge>
                    </div>
                  ))}
                </CardContent>
              </Card>
