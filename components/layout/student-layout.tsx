...
function StudentSidebar() {
  ...
  const navLinkClass = ({ isActive }: { isActive: boolean }) =>
    `flex items-center gap-3 px-3 py-2.5 rounded-[10px] text-sm font-medium transition-colors border ${
      isActive
  ...
  return (
    <aside className="w-64 shrink-0 flex flex-col h-screen bg-sidebar border-r border-sidebar-border">
      ...
      <div className="px-4 py-4 space-y-2">
        ...
        <button
          onClick={switchRole}
          className="w-full flex items-center gap-2 px-3 py-2 rounded-[10px] text-xs font-medium text-muted-foreground hover:text-foreground hover:bg-sidebar-accent transition-colors"
        >
